...
is_mongo_installed=$(which mongo)
if [ -z "$is_mongo_installed" ];
then
    echo "Mongo client is not installed on this machine. Initiating mongo installation"
    ./mongo-shell-install.sh
    exit 1
fi
...
cd $install_dir
# Create the LetsEncrypt certificate by creating a dummy certificate using Nginx docker
